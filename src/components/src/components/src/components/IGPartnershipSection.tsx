  ...
  return (
    <section className="py-20 bg-navy-800 border-y border-white/5">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col sm:flex-row items-center justify-center gap-8 mb-14 p-8 rounded-2xl border border-[#2D7A55]/20 bg-[#1B4D3E]/8">
          <a href="https://threadstylez.com" target="_blank" rel="noopener noreferrer" className="flex-shrink-0">
            <img src={TS_LOGO} alt="ThreadStylez" className="h-16 w-auto object-contain" onError={(e) => { (e.target as HTMLImageElement).src = '/image.png'; }} />
          </a>
          ...
          <a href="https://islamic-games.com" target="_blank" rel="noopener noreferrer" className="flex-shrink-0">
            <img src={IG_LOGO} alt="Islamic Games 2026" className="h-16 w-auto object-contain" onError={(e) => { const el = e.target as HTMLImageElement; el.style.display = 'none'; const div = document.createElement('div'); div.style.cssText = 'padding:12px 20px;background:#1B4D3E;border:1px solid #2D7A55;border-radius:12px;color:white;font-weight:900;font-size:16px;'; div.textContent = 'Islamic Games'; el.parentNode?.appendChild(div); }} />
          </a>
          <div className="hidden sm:block w-px h-16 bg-white/10" />
          <div className="text-center sm:text-left">
            <p className="font-display font-black text-white text-lg uppercase tracking-wide">ThreadStylezBrands.com</p>
            <p className="text-gray-400 text-sm mt-1">Dedicated Islamic Games apparel portal — launching 2026</p>
            <a href="https://threadstylezbrands.com" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-gold-500 hover:text-gold-400 text-xs font-display font-bold uppercase tracking-wider mt-2 transition-colors">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-14">
          {stats.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="card-surface p-5 text-center">
        ...
        <div className="mb-10">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {events.map((ev, i) => (
              <motion.a key={ev.city} href={ev.url} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }} className="group card-surface p-5 hover:border-[#2D7A55]/50 transition-all block">
            ...
            <div className="card-surface p-5 flex items-center justify-center border-dashed">
              <div className="text-center">
                ...
                <a href="https://islamic-games.com" target="_blank" rel="noopener noreferrer" className="text-[10px] text-gray-600 hover:text-gold-500 mt-1 inline-flex items-center gap-1 transition-colors">Check islamic-games.com <ExternalLink className="w-2.5 h-2.5" /></a>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-3 gap-5">
          {[
            { icon: Shirt, title: 'All Sports Covered', desc: 'Custom jerseys, hoodies, tracksuits, singlets, and accessories for every Islamic Games sport — basketball, soccer, cricket, track, swimming, volleyball, and more.' },
            { icon: Users, title: 'Teams & Individuals', desc: "Whether you're ordering for yourself or for 100+ athletes, we handle it all. Team bulk pricing (15% off 5+ items) and individual custom orders both supported." },
            { icon: Trophy, title: 'Official Quality', desc: "As the official provider, all ThreadStylez apparel meets the Islamic Games' standards for quality, durability, and professional presentation on the field." },
