// ─────────────────────────────────────────────────────────────
// RushOrderBanner — shown at top of page and in checkout.
// Reads from the shared rushOrder utility. No Stripe/API touch.
// ─────────────────────────────────────────────────────────────
...
interface Props {
  status: RushStatus;
  compact?: boolean; // true = single-line bar, false = full card
}
  ...
  if (compact) {
    return (
      <div className={`w-full ${s.bar} py-2.5 px-4 flex items-center justify-center gap-3 text-center`}>
        ...
        <span className={`text-xs font-display font-bold uppercase tracking-wider ${s.text}`}>
          {status.message}
        </span>
    ...
    <div className={`rounded-xl border ${s.bg} ${s.border} p-5`}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <p className={`font-display font-black text-sm uppercase tracking-wider leading-tight mb-1 ${s.text}`}>
            {status.message}
          </p>
          <p className={`text-xs font-body leading-relaxed ${s.sub}`}>
            {status.subMessage}
          </p>
          {status.isRush && (
